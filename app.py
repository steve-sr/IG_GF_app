import os, re, math, secrets, string, csv
...
def normalize_import_barrio_value(value):
    raw = (value or '').strip()
    if not raw:
        return 'Otro', 'Por definir'
    for barrio in BARRIOS_LIBERIA:
        if barrio.lower() == raw.lower():
            return barrio, None
    return 'Otro', import_title_name(raw)

def import_leaders_csv_path():
    custom_path = os.getenv('LEADERS_IMPORT_FILE')
    if custom_path:
        return Path(custom_path)
    return Path(__file__).resolve().parent / 'data' / 'lideres_import.csv'

@app.cli.command('import-leaders-cells')
def import_leaders_cells():
    """Importa líderes desde data/lideres_import.csv y crea su célula base.
    Es idempotente: actualiza por usuario y no duplica células.
    Las células nuevas quedan pausadas para que no salgan públicas hasta completar ubicación/día/hora.
    """
    db.create_all()
    path = import_leaders_csv_path()
    if not path.exists():
        print(f'No se encontró el archivo de importación: {path}')
        return
    ...
    with path.open('r', encoding='utf-8-sig', newline='') as f:
        ...
        for row in reader:
            raw_name = import_csv_value(row, 'LIDER', 'lider', 'name', 'Nombre')
            name = import_title_name(raw_name)
            raw_username = import_csv_value(row, 'USUARIO', 'usuario', 'username') or name
            username = slug_username(raw_username)
            raw_phone = import_csv_value(row, 'TELEFONO', 'telefono', 'phone')
            phone = format_cr_phone(raw_phone) if raw_phone else None
            password = import_csv_value(row, 'CONTRASENA', 'contraseña', 'password')
            if not password:
                digits = clean_phone(phone or raw_phone)
                password = f'{digits}!' if digits else random_password()
            barrio_raw = import_csv_value(row, 'BARRIO', 'barrio')
            barrio, barrio_other = normalize_import_barrio_value(barrio_raw)
            ...
            if not user:
                ...
                user.role = 'leader'
                user.active = True
                # Mantiene las credenciales del archivo para que el onboarding sea consistente.
                user.set_password(password)
                updated_users += 1
            ...
            if not cell:
                ...
                cell.barrio = barrio or cell.barrio or 'Otro'
                cell.barrio_other = barrio_other
                cell.phone = cell.phone or phone
                if not cell.address or str(cell.address).strip().lower() in ['none', '']:
                    cell.address = 'Dirección por definir'
                if not cell.day or str(cell.day).strip().lower() in ['none', '']:
                    cell.day = 'Por definir'
                if not cell.time or len(str(cell.time)) > 10 or str(cell.time).strip().lower() in ['none', 'por definir']:
                    cell.time = '19:00'
    ...
    db.session.commit()
    print('Importación de líderes y células completada.')
...
def import_group_addresses():
    ...
    for row in GROUP_ADDRESS_IMPORT_DATA:
        ...
        if not time or len(time) > 10 or time.lower() in ['por definir', 'pendiente']:
            time = '19:00'
        ...
        if not cell:
            ...
            cell.time = time or (cell.time if cell.time and len(cell.time) <= 10 else '19:00')
